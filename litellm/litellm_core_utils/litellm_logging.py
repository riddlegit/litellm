...
def _init_custom_logger_compatible_class(  # noqa: PLR0915
    ...
    try:
        ...
        elif logging_integration == "headroom":
            from litellm.integrations.headroom import HeadroomLogger

            for callback in _in_memory_loggers:
                if isinstance(callback, HeadroomLogger):
                    return callback  # type: ignore
            headroom_logger = HeadroomLogger()
            _in_memory_loggers.append(headroom_logger)
            return headroom_logger  # type: ignore
